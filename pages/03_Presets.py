...
import traceback
import os, sys, json, io, base64, requests
from datetime import datetime
from pathlib import Path
import streamlit as st

# Page config moet de eerste Streamlit-call zijn
st.set_page_config(page_title="Presets & GitHub", page_icon="📁", layout="wide")

# (optioneel) bootstrap als je die hebt
try:
    from bootstrap import ROOT  # repo-root op sys.path
except Exception:
...
def snapshot_from_session():
    """Pak relevante inputs uit session_state."""
    keys = [
...
def github_get_sha(owner, repo, branch, path, token):
    ...
    params = {"ref": branch}
    r = requests.get(url, headers=_auth_header(token), params=params, timeout=20)
    if r.status_code == 200:
        return r.json().get("sha")
    return None

def github_put_file(owner, repo, branch, path, token, content_bytes, message):
    """Create/update file via GitHub API (base64 content; include SHA als het al bestaat)."""
    url = f"https://api.github.com/repos/{owner}/{repo}/contents/{path}"
    ...
    headers = {"Accept":"application/vnd.github+json", **_auth_header(token)}
...
def github_get_raw(owner, repo, branch, path, token):
    url = f"https://api.github.com/repos/{owner}/{repo}/contents/{path}"
    r = requests.get(url, headers=_auth_header(token), params={"ref":branch}, timeout=20)
    if r.status_code != 200:
...
def main():
    ...
    st.caption("Gebruik deze pagina om **presets (JSON)** te bewaren of te laden, en om bestanden **rechtstreeks** naar GitHub te pushen.")
    ...
    with col_b:
        st.download_button("⬇️ Download preset (JSON)", data=json_bytes, file_name=fname or "preset.json", mime="application/json")
    ...
    st.caption("Vul je **repo**, **pad** en **token** in om bestanden te uploaden of bij te werken. Zet voor productie `GITHUB_TOKEN` in `.streamlit/secrets.toml`.")
    ...
    with col1: gh_owner  = st.text_input("Owner / org", default_owner or "gerrit0492-create")
    with col2: gh_repo   = st.text_input("Repo", default_repo or "maakindustrie-cost-tool-multipage")
    with col3: gh_branch = st.text_input("Branch", default_branch or "main")

    tok_col, path_col = st.columns([1,2])
    with tok_col:  gh_token = st.text_input("GitHub token (repo scope)", value=default_token, type="password")
    with path_col: target_path = st.text_input("Pad in repo (bijv. `presets/demo.json` of `exports/rapport.pdf`)", "presets/preset.json")
    ...
    with colg1: gh_path_load = st.text_input("Pad naar preset in repo", "presets/preset.json")
    with colg2: load_btn = st.button("⬇️ Laden en toepassen")
        ...
        if not gh_owner or not gh_repo or not gh_branch or not gh_path_load:
            ...
            if raw is None:
                ...
                try:
                    ...
                    st.error(f"JSON kon niet worden gelezen: {e}")
